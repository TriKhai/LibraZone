<template>
  <div class="product__card d-flex mb-1 mx-3 p-2" @click="handleClickProduct(book)">
    <div class="produt__img me-3">
      <img :src="book.image" alt="" @click="handleClickCard(book.book._id)" />
    </div>
    <div class="product__content">
      <div class="product__title">
        <p class="fs-4 fw-bold me-2 mb-0">{{ book.book.title }}</p>
        <span>({{ book.book.published_date }})</span>
      </div>
...
const handleClickProduct = async (record) => {
  await trackStore.setRecord(record)
}
</script>
